...
// Verify API.AI api call
module.exports.apiaiAuth = function(req, res, next) {
    ...
    // verifies secret and checks exp
    if(token == config.security.apiaiAuthKey) {
        next();
    } else {
        return res.json(failure('We cannot authenticate with this token!', HTTP.FORBIDDEN));
    }
};